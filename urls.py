...
def caixa_geral(request):
    ...
    if request.method == "POST":
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if dados:
                    paciente, profissional, especialidade = dados

                    # Definir valor (exemplo simples)
                    valor = 150.00

                    status = "Pago" if forma != "Convênio" else "Convênio"

                    cursor.execute("""
                        INSERT INTO caixa (
                            atendimento_id, paciente_nome, profissional_nome,
                            especialidade, valor, forma_pagamento, status, data_atendimento
                        )
                        VALUES (%s,%s,%s,%s,%s,%s,%s,CURRENT_DATE)
                    """, [
                        atendimento_id, paciente, profissional,
                        especialidade, valor, forma, status
                    ])
                    ...
                    mensagem = '<div class="alert alert-success">✅ Pagamento registrado!</div>'

        except Exception as e:
            mensagem = f'<div class="alert alert-danger">❌ Erro: {e}</div>'

    # --- 2. LISTAR ATENDIMENTOS PARA COBRANÇA ---
    with connection.cursor() as cursor:
        cursor.execute("""
            SELECT a.id, p.nome, pr.nome, e.nome, a.horario
            FROM agenda_diaria a
            JOIN pacientes p ON a.paciente_id = p.id
            JOIN profissionais pr ON a.profissional_id = pr.id
            JOIN especialidades e ON a.especialidade_id = e.id
            WHERE a.status = 'Aguardando Atendimento'
            ORDER BY a.horario
        """)
    ...
    conteudo = f"""
        <h4><i class="bi bi-cash-coin"></i> Caixa</h4><hr>
        {mensagem}

        <div class="row mb-4">
            <div class="col-md-6">
                <div class="p-3 bg-success text-white rounded">
                    <h5>Total Recebido Hoje</h5>
                    <h3>R$ {resumo[1]}</h3>
                </div>
            </div>
            <div class="col-md-6">
                <div class="p-3 bg-primary text-white rounded">
                    <h5>Atendimentos Pagos</h5>
                    <h3>{resumo[0]}</h3>
                </div>
            </div>
        </div>

        <h5>Pacientes para Cobrança</h5>
        <div class="table-responsive">
            <table class="table table-hover">
                <thead class="table-dark">
                    <tr>
                ...
                <tbody>
                    {linhas if atendimentos else '<tr><td colspan="5" class="text-center">Nenhum atendimento pendente.</td></tr>'}
                </tbody>
            </table>
        </div>
