...
# --- TELA 13: AGENDAMENTO ---
# --- TELA 13: AGENDAMENTO (FILTRO ESTRITO POR GRADE) ---
@csrf_exempt
def agendar_consulta(request):
    ...
    with connection.cursor() as cursor:
        # 1. FILTRO DE UNIDADES: Só mostra unidades que têm ALGUMA grade configurada
        cursor.execute("""
        ...
        # 2. FILTRO DE ESPECIALIDADES: Só mostra especialidades vinculadas à unidade escolhida e com grade
        especialidades = []
        if unid_id:
            cursor.execute("""
        ...
        # 3. FILTRO DE MÉDICOS: Só mostra médicos da unidade + especialidade que possuem grade
        profs_filtrados = []
        ...
        # 4. DATAS DISPONÍVEIS: Busca as datas que o médico tem grade aberta
        datas_disponiveis = []
        if prof_id and unid_id:
            cursor.execute("""
                SELECT DISTINCT data_especifica 
                FROM agendas_config 
    ...
    # --- LÓGICA DE POST E MONTAGEM DE HTML ---
        ...
        <div class="container py-3">
            ...
            <div class="card p-3 shadow-sm border-0 bg-light mb-4">
                <form method="GET" class="row g-2">
                    ...
                    <div class="col-md-3">
                        <label class="small fw-bold">3. Médico</label>
                    ...
                    <div class="col-md-3">
                        <label class="small fw-bold text-danger">4. Data Disponível</label>
                        <select name="data_sel" class="form-select border-danger" onchange="this.form.submit()" {"disabled" if not prof_id else ""}>
                            <option value="">Selecione...</option>{opts_datas}
                        </select>
                    </div>
                </form>
            </div>
            ...
            <div class="card p-3 shadow-sm mb-4 text-center">
                <h6 class="fw-bold">Escolha o Horário para {datetime.datetime.strptime(data_sel, "%Y-%m-%d").strftime("%d/%m/%Y") if data_sel else ""}</h6>
                <div class="d-flex flex-wrap justify-content-center">{btns_horas if btns_horas else '<span class="text-muted">Selecione uma data para ver horários</span>'}</div>
            </div>
            ''' if data_sel else ""}

            {''' ... resto do código de preenchimento de nome/whatsapp (POST) ... ''' if hora_sel else ""}
        </div>
    """
